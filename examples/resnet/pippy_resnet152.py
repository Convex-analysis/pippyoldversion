    ...
    if args.rank == 0:
        ...
        for epoch in range(args.max_epochs):
            ...
            for k, dataloader in loaders.items():
                ...
                total_epoch_samples += total_samples_processed
            ...
            print(f"Epoch {epoch + 1} execution time: {epoch_execution_time:.2f} seconds")
            print(f"Average memory usage: {avg_memory_usage:.2f} MB")
            
            # Write metrics to CSV
            with open(metrics_file, 'a', newline='') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow([epoch + 1, f"{epoch_execution_time:.2f}", total_epoch_samples, f"{avg_memory_usage:.2f}"])
