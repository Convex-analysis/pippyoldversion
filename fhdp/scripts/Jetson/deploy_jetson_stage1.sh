...
if [ "$IS_JETPACK" -eq 1 ]; then
    ...
    if [ -f fhdp/requirements/jetson.txt ]; then
        echo "   Using optimized Jetson requirements from fhdp/requirements/jetson.txt..."
    ...
    for req_file in fhdp/requirements/evo1.txt fhdp/requirements/ml.txt; do
        if [ -f "$req_file" ]; then
    ...
    python3 fhdp/scripts/install_evo1_deps.py
...
if [ $STAGED_INSTALL_RESULT -ne 0 ]; then
    ...
    if [ -f fhdp/requirements/jetson.txt ]; then
        ...
        if [ -f fhdp/requirements/base.txt ]; then
    ...
    bash fhdp/scripts/fix_flash_attn.sh || echo "   ⚠️  flash-attn failed (optional)"
