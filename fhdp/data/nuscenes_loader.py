...
class NuScenesDrivingLoader(Dataset):
    ...
    def __init__(
        ...
        logging.info(f"Initializing NuScenes data loader: {config.data_root}, {config.version}, {split}")
        ...
        logging.info(f"Loading scenes for split: {split}")
        self.scenes = self._filter_driving_scenes()
        logging.info(f"Total scenes after filtering: {len(self.scenes)}")
        
        # Federated learning: split data by client
        if client_id is not None and num_clients is not None:
            logging.info(f"Splitting scenes for client {client_id} out of {num_clients} clients")
            self.scenes = self._split_data_by_client(self.scenes, client_id, num_clients)
            logging.info(f"Client {client_id} assigned {len(self.scenes)} scenes")
        
        # Preprocess sequences
        logging.info("Creating sequences from scenes...")
        self.sequences = self._create_sequences()
        logging.info(f"Created {len(self.sequences)} training sequences")
        
        # Setup image transforms
        self.image_transform = self._setup_image_transforms()
        logging.info("Data loader initialization completed")
    ...
    def _filter_driving_scenes(self) -> List[Dict]:
        ...
        for scene in self.nusc.scene:
            # Enhanced filtering for driving relevance
            description = scene['description'].lower()
            
            # Check for driving-related keywords
            driving_keywords = [
                'driving', 'vehicle', 'traffic', 'road', 'highway', 'street',
                'intersection', 'turn', 'truck', 'bus', 'car', 'bicycle', 'scooter',
                'ped', 'pedestrian', 'crosswalk', 'parking', 'parked', 'jog',
                'construction', 'night', 'bike', 'cyclist'
            ]
            
            # Also check for common driving scenarios
            scenario_keywords = [
                'intersection', 'cross', 'turn left', 'turn right', 'following',
                'wait at', 'busy', 'street', 'highway', 'road'
            ]
            
            # Scene is driving-related if it contains any driving keyword
            if (any(keyword in description for keyword in driving_keywords) or
                any(keyword in description for keyword in scenario_keywords)):
                driving_scenes.append(scene)
        
        # Ensure we have enough driving scenes
        if len(driving_scenes) < 5:
            logging.info(f"Only {len(driving_scenes)} driving scenes found with filtering. Using all {len(self.nusc.scene)} scenes.")
            return self.nusc.scene  # Use all scenes for mini datasets
        
        logging.info(f"Found {len(driving_scenes)} driving scenes out of {len(self.nusc.scene)} total scenes")
    ...
    def _get_camera_image(self, sample_token: str, camera_channel: str) -> np.ndarray:
        ...
        sample_data = self.nusc.get('sample_data', self.nusc.get('sample', sample_token)['data'][camera_channel])
    ...
    def _get_vehicle_state(self, sample_token: str) -> Dict[str, np.ndarray]:
        ...
        # Get ego pose from one of the sample_data entries (e.g., CAM_FRONT)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for i in range(self.config.sequence_length):
            ...
            future_trajectory = []
            for t in range(1, self.model_config.max_waypoints + 1):
