...
def custom_cat(Tensor, dim):
    return torch.cat(Tensor, dim)
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor):
        ...
        dim_t = custom_arrange(self.num_pos_feats, dtype=torch.float32, device=x.device)
...
class Memfuser(nn.Module):
    ...
    def forward(self, x):
        ...
        tgt = self.position_encoding(
            custom_ones((bs, 1, 50, 50), device=x["rgb_front"].device)
        )
        tgt = tgt.flatten(2)
